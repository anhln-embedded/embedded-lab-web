...
import { ListTree, ArrowUp, Share2, Check, PanelRightClose, Zap } from "lucide-react";
...
export function CourseTableOfContents({
  ...
  return (
    <aside className="hidden xl:flex flex-col w-68 flex-shrink-0 sticky top-28 self-start max-h-[calc(100vh-8rem)] overflow-y-auto space-y-4 pl-2 pr-1 select-none scrollbar-thin">
      {/* XP Reward Card */}
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="text-xs font-black text-text-primary flex items-center gap-1.5">
              <span>+{earnedExp} XP {isEn ? "Reward" : "Thưởng"}</span>
              {isCompleted && (
                <span className="text-[10px] px-1.5 py-0.2 rounded-md bg-accent/15 text-accent border border-accent/30 font-bold">
            ...
            <p className="text-[11px] text-text-muted">
              {isCompleted
                ? (isEn ? "You completed this lesson" : "Bạn đã hoàn thành bài học này")
                : (isEn ? "Upon completing this lesson" : "Khi hoàn thành bài học này")}
            </p>
          </div>
        </div>
      </div>
        ...
        <>
          <div className="flex items-center justify-between pb-2 border-b border-border/60">
            <div className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-wider text-text-muted">
              ...
              <span>{isEn ? "Table of Contents" : "Mục Lục Bài Học"}</span>
            </div>
            ...
            {onCollapse && (
              <button
                ...
                title={isEn ? "Collapse TOC" : "Thu gọn mục lục"}
              >
                  ...
                  onClick={(e) => {
                    ...
                    if (!el) {
                      const allHeadings = document.querySelectorAll(
                        "#lesson-content-area h2, #lesson-content-area h3, article h2, article h3, main h2, main h3"
                      );
        ...
        >
          {copied ? (
            ...
            <>
              ...
              <span>{isEn ? "Share Lesson" : "Chia sẻ bài học"}</span>
            </>
          )}
        </button>
